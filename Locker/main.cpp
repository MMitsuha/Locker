...
ScsiReadWriteDisk(
	HANDLE hDisk,
	BOOLEAN IsRead,
	ULONG64 OffsetInSector,
	PVOID pBuffer,
	ULONG32 SizeOfBufferInSector,
...
)
{
	...
	DWORD Returned = 0;
	return LI_FN(DeviceIoControl)(hDisk, IOCTL_SCSI_PASS_THROUGH_DIRECT, &ScsiBuffer.ScsiCmd, sizeof(SCSI_PASS_THROUGH_DIRECT_WITH_BUFFER), &ScsiBuffer.ScsiCmd, sizeof(SCSI_PASS_THROUGH_DIRECT_WITH_BUFFER), &Returned, nullptr) && !ScsiBuffer.ScsiCmd.ScsiStatus;
}
	...
	if (DiskHandle != INVALID_HANDLE_VALUE)
	{
		...
		if (LI_FN(ReadFile)(DiskHandle, Sector, static_cast<DWORD>(sizeof(Sector)), &Bytes, nullptr))
		{
			...
			if (LI_FN(WriteFile)(DiskHandle, Sector, static_cast<DWORD>(sizeof(Sector)), &Bytes, nullptr))
			{
				...
				SPDLOG_DEBUG("WriteFile error,GetLastError(): {}", GetLastError());
		}
		else
			SPDLOG_DEBUG("ReadFile error,GetLastError(): {}", GetLastError());
#else
		...
		SPDLOG_DEBUG("CreateFile error,GetLastError(): {}", GetLastError());
